...
<p align="center">
This repo includes project-specific reference material under <code>docs/warehouse.md</code> (warehouse schema), <code>docs/reviewers/</code> (review checklists for Snowflake/UI work), and <code>.cortex/skills/</code> (curated context packs for the CAVM architecture, dbt conventions, and SEC pipeline patterns). These were built up over the semester so AI pair programming stayed useful on a codebase this size. The design choices, benchmark methodology, system integration, and debugging were ours. The tooling was a force multiplier, not the author.
</p>
